...
private:
    ...
    struct RoCCPendingEntry {
        VanadisInstruction* ins;
        uint32_t hw_thread;
    };
    std::vector<std::unordered_map<uint64_t, RoCCPendingEntry>> rocc_pending_;
    uint64_t rocc_next_cmd_id = 1;
    ...
    // OpenMP runtime library address range tracking for debugging
    uint64_t omp_runtime_start_addr;
    uint64_t omp_runtime_end_addr;
    bool track_omp_runtime;
    
    // Method to set OpenMP runtime address range
    void setOMPRuntimeRange(uint64_t start, uint64_t end) {
        omp_runtime_start_addr = start;
        omp_runtime_end_addr = end;
        output->verbose(CALL_INFO, 2, 0,
            "OpenMP runtime address range set: 0x%" PRIx64 " - 0x%" PRIx64 "\n",
            start, end);
    }
    
    // Check if address is in OpenMP runtime
    bool isInOMPRuntime(uint64_t addr) const {
        return track_omp_runtime &&
               addr >= omp_runtime_start_addr &&
               addr < omp_runtime_end_addr;
    }
