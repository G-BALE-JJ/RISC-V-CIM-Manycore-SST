    ...
    m_curRetireHwThread(0), m_curIssueHwThread(0), m_checkpointing(nullptr)
{
    ...
    // Initialize OpenMP runtime tracking
    omp_runtime_start_addr = 0;
    omp_runtime_end_addr = 0;
    track_omp_runtime = params.find<bool>("track_omp_runtime", false);
    
    if (track_omp_runtime) {
        output->verbose(CALL_INFO, 2, 0, "OpenMP runtime tracking enabled\n");
    }
    ...
    if (lists) {
        for (int i = 0; i <= lists->getMaxPopulatedSlotNumber(); i++) {
            if (lists->isPopulated(i)) {
                ...
                rocc_pending_.push_back(std::unordered_map<uint64_t, RoCCPendingEntry>());
            ...
            if ( ! ins->completedIssue() )
            {
                ...
                if ( 0 == resource_check ) // Resources available, can issue
                {
                        ...
                        ins_type == INST_ROCC0 || ins_type == INST_ROCC1 || ins_type == INST_ROCC2 || ins_type == INST_ROCC3)
                    {
                        ...
                        if (is_rocc && !ins->checkFrontOfROB()) {
                            allocate_fu = 1;
                        } else
                    ...
                    if ( 0 == allocate_fu )
                    {
                        ...
                        status = assignRegistersToInstruction(
                            ...
                            if ( output_verbosity >= 8 )
                            {
                                ins->printToBuffer(instPrintBuffer, 1024);
                                output->verbose(
                                    CALL_INFO, 0, 0, "%d: ----> Issued for: %s / 0x%" PRI_ADDR " / status: %d\n",
...
VANADIS_COMPONENT::performExecute(const uint64_t cycle)
{
    ...
    for (int i = 0; i < roccs_.size(); i++) {
        RoCCResponse* resp;
        if ((resp = roccs_[i]->respond())) {
            VanadisInstruction* matched_ins = nullptr;
            uint32_t matched_thr = 0;

            auto pending_it = rocc_pending_[i].find(resp->cmd_id);
            if (pending_it != rocc_pending_[i].end()) {
                VanadisInstruction* expected = pending_it->second.ins;
                matched_thr = pending_it->second.hw_thread;
                rocc_pending_[i].erase(pending_it);

                if (matched_thr < hw_threads) {
                    auto* thr_rob = rob[matched_thr];
                    for (uint32_t j = 0; j < thr_rob->size(); ++j) {
                        VanadisInstruction* cand = thr_rob->peekAt(j);
                        if (cand == expected) {
                            matched_ins = cand;
                            break;
                        }
            ...
            if (matched_ins != nullptr) {
                if (matched_ins->countISAIntRegOut() > 0 && matched_ins->countPhysIntRegOut() > 0 && resp->rd != 0) {
                    uint16_t phys_reg_index = matched_ins->getPhysIntRegOut(0);
                    const uint16_t reg_count = register_files[matched_ins->getHWThread()]->countIntRegs();
                    if (phys_reg_index < reg_count) {
                        register_files[matched_ins->getHWThread()]->setIntReg<uint64_t>(phys_reg_index, resp->rd_val);
                    }
                }
                matched_ins->markExecuted();
            } else {
                rocc_unmatched_resp++;
                if (rocc_unmatched_resp <= 10 || (rocc_unmatched_resp % 1000) == 0) {
                    output->verbose(CALL_INFO, 0, 0,
                        "[RoCC WARN] unmatched response count=%" PRIu64 " rocc=%d cmd=%" PRIu64 " hwthr=%" PRIu32 " rd=%" PRIu16 " val=0x%" PRIx64 "\n",
                        rocc_unmatched_resp,
                        i,
                        resp->cmd_id,
                        resp->hw_thread,
                        static_cast<uint16_t>(resp->rd),
                        resp->rd_val);
                }
            }
    ...
    VanadisRegisterStack* fp_regs, VanadisISATable* isa_table)
{
    ...
    if (ins->getInstFuncType() >= INST_ROCC0 && ins->getInstFuncType() <= INST_ROCC3) {
        ...
        if (!roccs_[rocc_index]->RoCCFull()) {
            ...
            const uint64_t cmd_id = rocc_next_cmd_id++;
            rocc_pending_[rocc_index][cmd_id] = RoCCPendingEntry{ins, ins->getHWThread()};
            roccs_[rocc_index]->push(new RoCCCommand(rocc_inst, rs1_val, rs2_val, cmd_id, ins->getHWThread()));
        }
    }
...
VANADIS_COMPONENT::clearROBMisspeculate(const uint32_t hw_thr)
{
    ...
    for (size_t i = 0; i < rocc_pending_.size(); ++i) {
        auto& pending = rocc_pending_[i];
        for (auto it = pending.begin(); it != pending.end(); ) {
            if (it->second.hw_thread == hw_thr) {
                it = pending.erase(it);
            } else {
                ++it;
            }
