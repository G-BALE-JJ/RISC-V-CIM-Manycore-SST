...
golem_status_t golemRunSingleCoreSoftmaxForCore(
    ...
    int core_id,
    uint64_t c_base_hbm_addr,
    int64_t n_tile_stride_bytes) {
    ...
    // Allocate GM buffers
    const uint64_t local_row_gm = gm_addr(core_id, LOCAL_LAYOUT.accum);
    ...
    for (int64_t row = 0; row < m; ++row) {
        ...
        for (int64_t n_tile = 0; n_tile < n_tiles; ++n_tile) {
            ...
            const uint64_t tile_hbm_base = c_base_hbm_addr + n_tile * n_tile_stride_bytes;
            const uint64_t row_hbm_addr = tile_hbm_base + row * block_n * sizeof(float);

            // Load row segment from HBM
            const uint64_t row_bytes = n_count * sizeof(float);
            dma_remote_load_to_gm(core_id, row_hbm_addr, local_tmp_gm, row_bytes);
            set_len(row_bytes);
