...
class SoftmaxPipelineWrapperTest(unittest.TestCase):
    def test_wrapper_defaults_do_not_reference_old_sst_install_paths(self):
        with open(os.path.join(SCRIPT_DIR, "run_noc_dma_softmax_pipeline.sh"), "r", encoding="utf-8") as source_file:
            source = source_file.read()

        self.assertNotIn("/data4/lishun/pkg/sst_install", source)
        self.assertNotIn("/data4/jjgong/local/sstelements", source)

    def test_wrapper_defaults_reference_current_local_paths(self):
        with open(os.path.join(SCRIPT_DIR, "run_noc_dma_softmax_pipeline.sh"), "r", encoding="utf-8") as source_file:
            source = source_file.read()

        self.assertIn("/data4/jjgong/local/sstcore", source)
