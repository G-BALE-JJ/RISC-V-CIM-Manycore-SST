...
public:
        ...
        return enqueueBufferTransfer(
            ...
            [this, arrayID, matrix, tag, callback = std::move(callback)]() {
                for (size_t index = 0; index < matrix.size(); ++index) {
                    setMatrixItem(static_cast<int32_t>(arrayID),
                                  static_cast<int32_t>(index), matrix[index]);
                }
                if (callback) {
                    callback(true, tag);
                }
        ...
        return enqueueBufferTransfer(
            ...
            [this, arrayID, input, tag, callback = std::move(callback)]() {
                for (size_t index = 0; index < input.size(); ++index) {
                    setVectorItem(static_cast<int32_t>(arrayID),
                                  static_cast<int32_t>(index), input[index]);
                }
                if (callback) {
                    callback(true, tag);
                }
    ...
    virtual bool writeOutputAsync(
        ...
        uint64_t tag,
        typename ComputeArray::BufferCallback callback) override {
        if (arrayID >= numArrays || output.size() != outputArraySize ||
            elemBytes == 0) {
            return false;
        }
        return enqueueBufferTransfer(
            ...
            [this, arrayID, output, tag, callback = std::move(callback)]() {
                outputVectors[arrayID].resize(output.size());
                std::transform(output.begin(), output.end(), outputVectors[arrayID].begin(),
                               [](double value) { return static_cast<T>(value); });
                if (npArrayOut[arrayID] != nullptr) {
                    T* data = reinterpret_cast<T*>(PyArray_DATA(npArrayOut[arrayID]));
                    std::copy(outputVectors[arrayID].begin(),
                              outputVectors[arrayID].end(), data);
                }
                if (callback) {
                    callback(true, tag);
                }
