...
inline bool vanadisEnvFlagDefault(const char* name, bool defaultValue)
{
...
class VanadisBasicLoadStoreQueue : public SST::Vanadis::VanadisLoadStoreQueue
{
    public:
        ...
        SST_ELI_DOCUMENT_PARAMS(
                ...
                { "llsc_backoff_enabled", "Enable exponential backoff for failed LLSC operations", "true"},
                { "llsc_backoff_base_cycles", "Base delay cycles for LLSC backoff", "10"},
                { "llsc_backoff_max_exp", "Maximum exponent for LLSC backoff (max delay = base * 2^max_exp)", "10"},
                { "llsc_core_offset_cycles", "Per-core deterministic offset to prevent synchronization (cycles per core)", "50"},
                { "llsc_trace", "Enable per-operation LLSC trace lines", "false"}
            )
        ...
        llsc_trace(params.find<bool>("llsc_trace", vanadisEnvFlagDefault("GOLEM_LLSC_TRACE", false)))
        {
            ...
            // LLSC backoff parameters
            llsc_backoff_enabled = params.find<bool>("llsc_backoff_enabled", true);
            llsc_backoff_base_cycles = params.find<uint64_t>("llsc_backoff_base_cycles", 10);
            llsc_backoff_max_exp = params.find<uint32_t>("llsc_backoff_max_exp", 10);
            llsc_core_offset_cycles = params.find<uint64_t>("llsc_core_offset_cycles", 50);
            
            // Initialize per-thread backoff state
            llsc_backoff_counters.resize(hw_threads, 0);
            llsc_failure_counts.resize(hw_threads, 0);
            llsc_backoff_delays.resize(hw_threads, 0);

            output->verbose(CALL_INFO, 2, 0, "LLSC backoff: %s (base=%lu cycles, max_exp=%u, core_offset=%lu cycles)\n",
                llsc_backoff_enabled ? "enabled" : "disabled",
                llsc_backoff_base_cycles, llsc_backoff_max_exp, llsc_core_offset_cycles);
        ...
        {
            // Decrement LLSC backoff counters
            if (llsc_backoff_enabled) {
                for (int t = 0; t < hw_threads; t++) {
                    if (llsc_backoff_counters[t] > 0) {
                        llsc_backoff_counters[t]--;
                        if (llsc_trace && llsc_backoff_counters[t] == 0) {
                            output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                "[LLSC-BACKOFF-COMPLETE] thr=%d backoff period ended, can retry LLSC\n", t);
                        }
                    }
                }
            }
    ...
    protected:
        ...
        class StandardMemHandlers : public Interfaces::StandardMem::RequestHandler
        {
            public:
                ...
                {
                    if (lsq->llsc_trace) {
                        out->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_STORE_FLG,
                            "[LLSC-SC-RESPONSE] ins_addr=0x%" PRI_ADDR " thr=%d\n",
                            store_ins->getInstructionAddress(), store_ins->getHWThread());
                    }
                    const uint16_t value_reg = store_ins->getPhysIntRegOut(0);
                    const uint32_t hw_thread = store_ins->getHWThread();
                    ...
                    if (ev->getSuccess()) {
                        const int64_t success_result = store_cond_ins->getResultSuccess();
                            ...
                            if (lsq->llsc_trace) {
                                out->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_STORE_FLG,
                                                "[LLSC-SC-SUCCESS] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " result_reg=%" PRIu16 " value=%" PRId64 " (SC succeeded, reservation held)\n",
                                                store_ins->getInstructionAddress(), hw_thread,
                                                store_entry->getStoreAddress(), value_reg, success_result);
                            }
                        lsq->registerFiles->at(hw_thread)->setIntReg<int64_t>(value_reg, success_result);
                        
                        // Reset backoff on success
                        lsq->llsc_failure_counts[hw_thread] = 0;
                        lsq->llsc_backoff_delays[hw_thread] = 0;
                    } else {
                        const int64_t failure_result = store_cond_ins->getResultFailure();

                        // Increment failure count and calculate backoff delay with random jitter + core offset
                        if (lsq->llsc_backoff_enabled) {
                            lsq->llsc_failure_counts[hw_thread]++;
                            uint32_t exp = std::min(lsq->llsc_failure_counts[hw_thread], lsq->llsc_backoff_max_exp);
                            uint64_t base_delay = lsq->llsc_backoff_base_cycles * (1ULL << exp);
                            
                            // Add random jitter: 0% to 50% of base_delay
                            // Use hw_thread and failure count as seed for deterministic but varied jitter
                            uint64_t jitter_seed = (hw_thread * 7919 + lsq->llsc_failure_counts[hw_thread] * 6151) % 100;
                            uint64_t jitter = (base_delay * jitter_seed) / 200;  // 0-50% jitter
                            
                            // Add deterministic core offset to ensure cores retry at different times
                            uint64_t core_offset = hw_thread * lsq->llsc_core_offset_cycles;
                            
                            uint64_t delay = base_delay + jitter + core_offset;
                            
                            lsq->llsc_backoff_delays[hw_thread] = delay;
                            
                            if (lsq->llsc_trace) {
                                out->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_STORE_FLG,
                                                "[LLSC-SC-FAILED] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " result_reg=%" PRIu16 " value=%" PRId64 " failures=%u backoff=%lu cycles (base=%lu+jitter=%lu+core_offset=%lu) (SC failed, reservation lost - RETRY with BACKOFF)\n",
                                                store_ins->getInstructionAddress(), hw_thread,
                                                store_entry->getStoreAddress(), value_reg, failure_result,
                                                lsq->llsc_failure_counts[hw_thread], delay, base_delay, jitter, core_offset);
                            }
                        } else {
                            if (lsq->llsc_trace) {
                                out->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_STORE_FLG,
                                                "[LLSC-SC-FAILED] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " result_reg=%" PRIu16 " value=%" PRId64 " (SC failed, reservation lost - RETRY NEEDED)\n",
                                                store_ins->getInstructionAddress(), hw_thread,
                                                store_entry->getStoreAddress(), value_reg, failure_result);
                            }
                        }
                        
                        lsq->registerFiles->at(hw_thread)->setIntReg<uint64_t>(value_reg, failure_result);
                    }
                }
            ...
            VanadisStoreInstruction* store_ins)
        {
            ...
            case MEM_TRANSACTION_LLSC_STORE:
            {
                if(UNLIKELY(needs_split)) {
                    ...
                    if (llsc_trace) {
                        output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_STORE_FLG,
                            "[LLSC-SC-ISSUE] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " width=%" PRIu64 " (issuing StoreConditional)\n",
                            store_ins->getInstructionAddress(), store_ins->getHWThread(), store_address, store_width);
                    }
        ...
        void issueLoad(VanadisLoadInstruction* load_ins, uint64_t load_address, uint64_t load_width) {
            ...
            switch (load_ins->getTransactionType()) {
                ...
                case MEM_TRANSACTION_LLSC_LOAD:
                {
                    if(UNLIKELY(needs_split)) {
                        output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                            "[LLSC-LL-ERROR] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " SPLIT across cache line - NOT ALLOWED\n",
                            load_ins->getInstructionAddress(), load_ins->getHWThread(), load_address);
                        load_ins->flagError();
                    } else {
                        if (llsc_trace) {
                            output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                "[LLSC-LL-ISSUE] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " width=%" PRIu64 " (issuing LoadLink, setting reservation)\n",
                        ...
                        std::vector<uint64_t>& load_addresses, std::vector<uint16_t>& load_widths )
        {
            ...
            else
            {
                ...
                else if((load_ins->getTransactionType() == MEM_TRANSACTION_LLSC_LOAD) || (load_ins->getTransactionType() == MEM_TRANSACTION_LOCK))
                {
                    ...
                    // Check if thread is in backoff period
                    if (llsc_backoff_enabled && llsc_backoff_counters[hw_thr] > 0)
                    {
                        if (llsc_trace) {
                            output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                "[LLSC-LL-BACKOFF] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " in backoff period (%lu cycles remaining)\n",
                                load_ins->getInstructionAddress(), hw_thr, load_address, llsc_backoff_counters[hw_thr]);
                        }
                        return false;
                    }
                    
                    if (!stores_pending[hw_thr].empty())
                    {
                        if (llsc_trace) {
                            output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                "[LLSC-LL-BLOCKED] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " BLOCKED by pending stores (must drain store queue first)\n",
                                load_ins->getInstructionAddress(), hw_thr, load_address);
                        }
                        return false;
                    }
                    else
                    {
                        // Check if there's a pending backoff delay from a previous SC failure
                        // If so, start the backoff period NOW and block this LL from issuing
                        if (llsc_backoff_enabled && llsc_backoff_delays[hw_thr] > 0) {
                            llsc_backoff_counters[hw_thr] = llsc_backoff_delays[hw_thr];
                            llsc_backoff_delays[hw_thr] = 0;  // Clear the delay marker
                            if (llsc_trace) {
                                output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                    "[LLSC-LL-BACKOFF-START] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " starting backoff period (%lu cycles) - BLOCKING LL\n",
                                    load_ins->getInstructionAddress(), hw_thr, load_address, llsc_backoff_counters[hw_thr]);
                            }
                            return false;  // Block the LL from issuing during backoff
                        } else {
                            if (llsc_trace) {
                                output->verbose(CALL_INFO, 0, VANADIS_DBG_LSQ_LOAD_FLG,
                                    "[LLSC-LL-READY] ins_addr=0x%" PRI_ADDR " thr=%d addr=0x%" PRI_ADDR " ready to issue (no pending stores, no backoff)\n",
                                    load_ins->getInstructionAddress(), hw_thr, load_address);
                            }
                        }
        ...
        // LLSC backoff mechanism
        bool llsc_backoff_enabled;
        uint64_t llsc_backoff_base_cycles;
        uint32_t llsc_backoff_max_exp;
        uint64_t llsc_core_offset_cycles;              // Per-core deterministic offset
        std::vector<uint64_t> llsc_backoff_counters;   // Current backoff countdown per thread
        std::vector<uint32_t> llsc_failure_counts;     // Consecutive SC failures per thread
        std::vector<uint64_t> llsc_backoff_delays;     // Calculated backoff delay per thread
};
