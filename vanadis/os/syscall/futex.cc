...
{
    // Enhanced Futex debugging for barrier deadlock detection
    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
            "[FUTEX-ENTRY] tid=%d pid=%d addr=%#" PRIx64 " op=%#x(%s) val=%#" PRIx32 " timeAddr=%#" PRIx64 " callStackAddr=%#" PRIx64
            " addr2=%#" PRIx64 " val3=%#x\n",
            process->gettid(), process->getpid(), event->getAddr(), event->getOp(),
            (event->getOp() & 0x7f) == FUTEX_WAIT ? "WAIT" :
            (event->getOp() & 0x7f) == FUTEX_WAKE ? "WAKE" :
            (event->getOp() & 0x7f) == FUTEX_REQUEUE ? "REQUEUE" : "OTHER",
            event->getVal(), event->getTimeAddr(), event->getStackPtr(), event->getAddr2(), event->getVal3() );
    ...
    switch ( m_op ) {
      case FUTEX_WAKE:
        {
            m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                "[FUTEX-WAKE-START] tid=%d addr=%#" PRIx64 " val=%d (max threads to wake)\n",
                process->gettid(), event->getAddr(), event->getVal());
            futexWake(event);
        } break;
      ...
      case FUTEX_WAIT:
        {
            m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                "[FUTEX-WAIT-START] tid=%d addr=%#" PRIx64 " expected_val=%d do LoadLink (LLSC)\n",
                process->gettid(), event->getAddr(), event->getVal());
      ...
      case FUTEX_REQUEUE:
        {
            m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                "[FUTEX-REQUEUE-START] tid=%d addr=%#" PRIx64 " val=%d\n",
                process->gettid(), event->getAddr(), event->getVal());
            m_buffer.resize(sizeof(uint32_t));
...
void VanadisFutexSyscall::wakeWaiter(VanadisSyscallFutexEvent* event) const
{
    ...
    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                      "[FUTEX-WAKE-WAITER] waker_tid=%d addr=%#" PRIx64 " found waiter_tid=%d, waking up\n",
...
int VanadisFutexSyscall::getNumWaitersToWake(VanadisSyscallFutexEvent* event) const
{
    ...
    {
        m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                          "[FUTEX-WAKE-NONE] tid=%d addr=%#" PRIx64 " NO WAITERS (possible spurious wake or race)\n",
                          m_process->gettid(), event->getAddr());
    }
    else
    {
        m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                          "[FUTEX-WAKE-COUNT] tid=%d addr=%#" PRIx64 " total_waiters=%u requested_wake=%u will_wake=%u\n",
...
{
    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
        "[FUTEX-WAIT-WAKEUP] tid=%d woken up by another thread, returning from wait\n",
        m_process->gettid());
    setReturnSuccess(0);
}
...
{
    m_output->verbose(CALL_INFO, 8, VANADIS_OS_DBG_SYSCALL,
        "[FUTEX-MEM-DONE] tid=%d failed=%d op=%d\n", m_process->gettid(), failed, m_op);
    switch( m_op ) {
      case FUTEX_WAIT:
      case FUTEX_WAIT_BITSET:
        {
            if ( ! m_waitStoreConditional ) {
                ...
                if ( m_val == getEvent<VanadisSyscallFutexEvent*>()->getVal() ) {
                    ...
                    // Issue store conditional (LLSC pattern for atomic check)
                    writeMemory( getEvent<VanadisSyscallFutexEvent*>()->getAddr(), m_buffer, true );

                    m_waitStoreConditional = true;

                    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                        "[FUTEX-WAIT-LLSC-SC] tid=%d addr=%#" PRIx64 " LoadLink read=%d expected=%d MATCH, issuing StoreConditional\n",
                        m_process->gettid(), getEvent<VanadisSyscallFutexEvent*>()->getAddr(),
                        m_val, getEvent<VanadisSyscallFutexEvent*>()->getVal());

                } else {
                    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                        "[FUTEX-WAIT-MISMATCH] tid=%d addr=%#" PRIx64 " read_val=%d expected_val=%d MISMATCH, returning EAGAIN (no wait)\n",
                ...
                if ( failed ) {
                    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                        "[FUTEX-WAIT-SC-FAIL] tid=%d addr=%#" PRIx64 " StoreConditional FAILED (race detected), returning EAGAIN\n",
                        m_process->gettid(), getEvent<VanadisSyscallFutexEvent*>()->getAddr());
                    setReturnFail(-LINUX_EAGAIN);
                } else {
                    m_output->verbose(CALL_INFO, 2, VANADIS_OS_DBG_SYSCALL,
                        "[FUTEX-WAIT-SLEEP] tid=%d addr=%#" PRIx64 " StoreConditional SUCCESS, adding to wait queue (BLOCKING)\n",
                        m_process->gettid(), getEvent<VanadisSyscallFutexEvent*>()->getAddr());
      ...
      case FUTEX_REQUEUE:
        {
            if ( ReadAddr == m_state ) {
                ...
                m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL, "[syscall-futex] FUTEX_REQUEUE wakeup %d waiter\n",val);

                int numWaiters = m_process->futexGetNumWaiters( getEvent<VanadisSyscallFutexEvent*>()->getAddr() );
                m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL, "[syscall-futex] FUTEX_REQUEUE numWaiters %d\n",numWaiters);
                ...
                for ( int i = 0; i < val && i < numWaiters; i++ ) {
                    ...
                    if ( syscall ) {
                        ++m_numWokeup;
                        m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL, "[syscall-futex] FUTEX_REQUEUE tid=%d addr=%#" PRIx64 " wakeup tid=%d\n",
                                    m_process->gettid(), getEvent<VanadisSyscallFutexEvent*>()->getAddr(), syscall->getTid());
                        dynamic_cast<VanadisFutexSyscall*>( syscall )->wakeup();
                        delete syscall;
                    }
                }
                m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL, "[syscall-futex] FUTEX_REQUEUE numWokeup %d\n",m_numWokeup);
...
{
    m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL, "[syscall-futex] FUTEX_REQUEUE read val2=%d addr2=%#" PRIx64 "\n",val2,addr2);
    ...
    for ( int i = 0; i < val2 && i < numWaiters; i++ ) {
        VanadisSyscall* syscall = m_process->findFutex( getEvent<VanadisSyscallFutexEvent*>()->getAddr() );
        if ( syscall ) {
            m_output->verbose(CALL_INFO, 0, VANADIS_OS_DBG_SYSCALL,"[syscall-futex] FUTEX_REQUEUE tid=%d addr=%#" PRIx64 " move to %#" PRIx64 "\n",m_process->gettid(),
