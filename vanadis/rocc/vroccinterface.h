...
{
public:
    RoCCCommand(RoCCInstruction* inst, uint64_t rs1, uint64_t rs2, uint64_t cmd_id = 0, uint32_t hw_thread = 0) {
        this->inst = inst;
        this->rs1 = rs1;
        this->rs2 = rs2;
        this->cmd_id = cmd_id;
        this->hw_thread = hw_thread;
    }
    ~RoCCCommand() {}
...
{
public:
    RoCCResponse(uint8_t rd, uint64_t rd_val, uint64_t cmd_id = 0, uint32_t hw_thread = 0) {
        this->rd = rd;
        this->rd_val = rd_val;
        this->cmd_id = cmd_id;
        this->hw_thread = hw_thread;
...
public:
    ...
    VanadisRoCCInterface(ComponentId_t id, Params& params) : SubComponent(id) {
        ...
        output = new SST::Output("[RoCC @t]: ", verbose_level, 0xFFFFFFFF, SST::Output::STDOUT);
