...
void LinkControl::finish(void)
{
    ...
    if (!g_packet_latency_summary_printed && !g_packet_latencies_ns.empty()) {
        ...
        const uint64_t avg = static_cast<uint64_t>(sum / g_packet_latencies_ns.size());
        const uint64_t p95 = percentile_of_sorted(g_packet_latencies_ns, 0.95);
        const uint64_t p99 = percentile_of_sorted(g_packet_latencies_ns, 0.99);
        const uint64_t maxv = g_packet_latencies_ns.back();
        output.output("MERLIN_PACKET_LATENCY_GLOBAL count=%zu avg_ns=%" PRIu64 " p95_ns=%" PRIu64 " p99_ns=%" PRIu64 " max_ns=%" PRIu64 "\n",
