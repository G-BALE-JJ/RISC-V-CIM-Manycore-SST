...
#define MEMBACKENDCONVERTOR_ELI_STATS { "cycles_with_issue",                  "Total cycles with successful issue to back end",   "cycles",   1 },\
    ...
    public:
        ...
        void setQueueEnterCycle(uint64_t cycle) { queueEnterCycle_ = cycle; }
        uint64_t getQueueEnterCycle() const { return queueEnterCycle_; }
        bool hasIssueCycle() const { return issueCycleValid_; }
        void setIssueCycle(uint64_t cycle) { issueCycle_ = cycle; issueCycleValid_ = true; }
        uint64_t getIssueCycle() const { return issueCycle_; }
        ...
        virtual void serialize_order(SST::Core::Serialization::serializer& ser) override {
            SST_SER(m_reqId);
            SST_SER(m_type);
            SST_SER(queueEnterCycle_);
            SST_SER(issueCycle_);
            SST_SER(issueCycleValid_);
        }
    ...
    protected:
        uint32_t m_reqId;
        ReqType m_type;
        uint64_t queueEnterCycle_ = 0;
        uint64_t issueCycle_ = 0;
        bool issueCycleValid_ = false;
    };
