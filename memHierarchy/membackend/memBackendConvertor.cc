...
namespace {
std::vector<uint64_t> g_mem_queue_delays_cycles;
bool g_mem_queue_summary_printed = false;
...
void MemBackendConvertor::handleCustomEvent( Interfaces::StandardMem::CustomData * info, Event::id_type evId, std::string rqstr) {
    ...
    req->setQueueEnterCycle(m_cycleCount);
...
bool MemBackendConvertor::clock(Cycle_t cycle) {
    ...
    while ( !m_requestQueue.empty()) {
        ...
        if ( issue( req ) ) {
            cycleWithIssue = true;
            if (!req->hasIssueCycle()) {
                req->setIssueCycle(m_cycleCount);
                g_mem_queue_delays_cycles.push_back(m_cycleCount - req->getQueueEnterCycle());
            }
        } else {
...
void MemBackendConvertor::finish(Cycle_t endCycle) {
    ...
    if (!g_mem_queue_summary_printed && !g_mem_queue_delays_cycles.empty()) {
        std::sort(g_mem_queue_delays_cycles.begin(), g_mem_queue_delays_cycles.end());
        unsigned long long sum = 0;
        for (auto v : g_mem_queue_delays_cycles) sum += v;
        const uint64_t avg = static_cast<uint64_t>(sum / g_mem_queue_delays_cycles.size());
        const uint64_t p95 = percentile_of_sorted(g_mem_queue_delays_cycles, 0.95);
        const uint64_t p99 = percentile_of_sorted(g_mem_queue_delays_cycles, 0.99);
        const uint64_t maxv = g_mem_queue_delays_cycles.back();
        std::cout << "MEM_QUEUE_DELAY_GLOBAL count=" << g_mem_queue_delays_cycles.size()
                  << " avg_cycles=" << avg
                  << " p95_cycles=" << p95
                  << " p99_cycles=" << p99
                  << " max_cycles=" << maxv << std::endl;
        g_mem_queue_summary_printed = true;
    }
