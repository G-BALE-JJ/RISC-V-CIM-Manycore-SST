...
MemNIC::MemNIC(ComponentId_t id, Params &params, TimeConverter* tc) : MemNICBase(id, params, tc) {
    ...
    link_control = loadUserSubComponent<SimpleNetwork>("linkcontrol", ComponentInfo::SHARE_NONE, num_vns);
    if (!link_control) {
        Params netparams;
        ...
        link_control = loadAnonymousSubComponent<SimpleNetwork>(link_control_class, "linkcontrol", 0, ComponentInfo::SHARE_PORTS | ComponentInfo::INSERT_STATS, netparams, num_vns);
    }
...
void MemNIC::send(MemEventBase *ev) {
    ...
    if (sendQueue.size() == 1)
        drainQueue(&sendQueue, link_control);
    if (sendQueue.size() == 1 && !clockOn) { /* Attempt again in 1 cycle */
        clockOn = true;
        reregisterClock(clockTC, clockHandler);
    }
}
